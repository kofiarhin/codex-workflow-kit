...
| Worker ID | Role | Branch | Worktree Path | Run ID | Current Task | Changed Files | Verification Result | Blocker | Final Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<worker-id>` | `<orchestrator / worker>` | `<branch>` | `<absolute-path>` | `<run-id>` | `<TASK-000 or none>` | `<paths or none>` | `<pending / passed / failed / unable-to-run>` | `<none or details>` | `<pending / done / blocked / needs-review>` |
