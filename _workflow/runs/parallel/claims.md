...
| Task ID | Worker ID | Branch | Worktree Path | Run ID | Claim Status | Start Time | End Time | Files Expected | Files Changed | Verification Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `<TASK-000>` | `<worker-id>` | `<branch>` | `<absolute-path>` | `<run-id>` | `<unclaimed / claimed / in-progress / done / blocked / needs-review>` | `<YYYY-MM-DD HH:MM>` | `<YYYY-MM-DD HH:MM or pending>` | `<paths>` | `<paths or pending>` | `<pending / passed / failed / unable-to-run>` |
